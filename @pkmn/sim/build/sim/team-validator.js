...
class TeamValidator {
    ...
    validateSet(set, teamHas) {
        ...
        let item = dex.items.get(lib_1.Utils.getString(set.item));
        set.item = item.name;
        let ability = dex.abilities.get(lib_1.Utils.getString(set.ability));
        set.ability = ability.name;
        let nature = dex.natures.get(lib_1.Utils.getString(set.nature));
        ...
        for (const moveName of set.moves) {
            if (!moveName)
                continue;
            const move = dex.moves.get(lib_1.Utils.getString(moveName));
            if (!move.exists)
        ...
        if (!setSources.sourcesBefore && setSources.sources.length) {
            ...
            for (const event of eventData) {
                if (this.validateEvent(set, event, eventSpecies))
                    continue;
                legal = true;
                break;
            }
    ...
    validateEvent(set, eventData, eventSpecies, because = ``, from = `from an event`) {
        ...
        let name = set.species;
        const species = dex.species.get(set.species);
        const maxSourceGen = this.ruleTable.has('allowtradeback') ? lib_1.Utils.clampIntRange(dex.gen + 1, 1, 8) : dex.gen;
        if (!eventSpecies)
    ...
    allSources(species) {
        ...
        const maxSourceGen = this.ruleTable.has('allowtradeback') ? lib_1.Utils.clampIntRange(this.dex.gen + 1, 1, 8) : this.dex.gen;
        return new PokemonSources(maxSourceGen, minSourceGen);
    }
    validateMoves(species, moves, setSources, set, name = species.name) {
        const dex = this.dex;
        const ruleTable = this.ruleTable;
        const problems = [];
