    ...
    */
    ...
    transformInto(pokemon, effect) {
        ...
        for (boostName in pokemon.boosts) {
            ...
            if (this.battle.gen <= 1) {
                if (boostName === 'evasion' || boostName === 'accuracy')
                    continue;
    ...
    hasItem(item) {
        ...
        if (!Array.isArray(item))
            return ownItem === (0, dex_1.toID)(item);
        return item.map(dex_1.toID).includes(ownItem);
    }
    clearItem() {
        return this.setItem('');
    }
    ...
    hasAbility(ability) {
        ...
        if (!Array.isArray(ability))
            return ownAbility === (0, dex_1.toID)(ability);
        return ability.map(dex_1.toID).includes(ownAbility);
    }
    clearAbility() {
