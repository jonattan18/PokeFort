...
module.exports.run = async (bot, interaction, user_available, pokemons) => {
    ...
    /*
    // Admin
    if (interaction.isadmin && interaction.AdminServer == interaction.guild.id) {
    ...
    user_model.findOne({ UserID: interaction.user.id }, (err, user) => {
        ...
        getPokemons.getallpokemon(interaction.user.id).then(pokemons_from_database => {
            ...
            if (interaction.options.get("latest") != null && interaction.options.get("id") != null) return interaction.reply({ content: "No pokémon exists with that number.", ephemeral: true });

            // If no arguments
            if (interaction.options.get("id") == null && interaction.options.get("latest") == null) {
                var selected_pokemon = user_pokemons.filter(it => it._id == user.Selected)[0];
                var index = user_pokemons.indexOf(selected_pokemon) + 1;
            }

            // If arguments is latest or l
            else if (interaction.options.get("latest") != null && interaction.options.get("id") == null) {
                var selected_pokemon = user_pokemons[user_pokemons.length - 1];
                var index = user_pokemons.length;
            }

            // If arguments is number
            else if (interaction.options.get("id") != null && interaction.options.get("latest") == null) {
                var user_index = interaction.options.get("id").value;
                if (typeof user_pokemons[user_index - 1] != 'undefined') {
                    var selected_pokemon = user_pokemons[user_index - 1];
                    var index = user_index;
                }
                else {
                    return interaction.reply({ content: "No pokémon exists with that number.", ephemeral: true });
                }
            }
            ...
            embed.setFooter({ text: `Displaying Pokémon: ${index}/${user_pokemons.length}` });
...
module.exports.config = {
    ...
    options: [
        {
            name: "id",
            description: "Show information based on ID of pokemon.",
            min_value: 1,
            type: 4
        },
        {
            name: "latest",
            description: "Show information based on latest pokemon.",
            type: 3,
            choices: [{
                name: "yes",
                value: "yes",
