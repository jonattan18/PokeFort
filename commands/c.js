...
function add(bot, message, args, pokemons, channel_data, user_data) {
    ...
    if (current_user == 1) {
        channel_model.findOneAndUpdate({ ChannelID: message.channel.id }, { $set: { "Trade.Credits.User1": new_credit } }, { upsert: true }, (err, channel) => {
            if (err) return console.log(err);
            if (!channel) return;
        });
    } else {
        channel_model.findOneAndUpdate({ ChannelID: message.channel.id }, { $set: { "Trade.Credits.User2": new_credit } }, { upsert: true }, (err, channel) => {
            if (err) return console.log(err);
            if (!channel) return;
        });
    }
