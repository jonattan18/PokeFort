...
module.exports.run = async (bot, message, args, prefix) => {
    ...
    if (args.length == 0) {
        ...
        json_data = help_data.filter(x => x.aliases != undefined && x.aliases.includes(args.join(" ")))[0];
    }

    if (json_data == null || json_data == undefined) return message.channel.send("No help found for that command.");
