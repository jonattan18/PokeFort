...
function add(interaction, prompt, user_data) {
    ...
    if (interaction.user.id == prompt.UserID.User1ID) {
        ...
        var old_credit = prompt.Trade.Shards.User1 == undefined ? 0 : prompt.Trade.Shards.User1;
    } else {
        current_user = 2;
        if (prompt.Trade.User2IConfirm == true) return interaction.reply({ content: 'You already confirmed your trade.', ephemeral: true });
        var old_credit = prompt.Trade.Shards.User2 == undefined ? 0 : prompt.Trade.Shards.User2;
    }
    ...
    if (new_credit > user_data.Shards || user_data.Shards == undefined || user_data.Shards == NaN) return interaction.reply({ content: 'You do not have enough Shards!', ephemeral: true });

    if (current_user == 1) {
        prompt.Trade.User2IConfirm = false;
        prompt.Trade.Shards.User1 = new_credit;
        prompt.save();
    } else {
        prompt.Trade.User1IConfirm = false;
        prompt.Trade.Shards.User2 = new_credit;
        prompt.save();
    }
...
module.exports.config = {
    name: "s",
    description: "Trade shards.",
    options: [{
        name: "add",
        description: "Add shards to trade.",
        type: 1,
        options: [{
            name: "amount",
            description: "Amount of shard to add.",
            type: 4,
            required: true,
            min_value: 1
        }]
        ...
        name: "remove",
        description: "Remove shards from trade.",
        type: 1,
        options: [{
            name: "amount",
            description: "Amount of shards to remove.",
            type: 4,
            required: true,
            min_value: 1
        }]
