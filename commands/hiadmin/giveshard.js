...
module.exports.run = async (bot, message, args, prefix, user_available, pokemons) => {
    ...
    user_model.findOne({ UserID: message.author.id }, (err, user) => {
        ...
        user_model.findOne({ UserID: user_id }, (err, user_2) => {
            ...
            user_model.findOneAndUpdate({ UserID: user_id }, { $inc: { Shards: amount } }, (err, user_3) => {
                if (err) return console.log(err);
                bot.users.fetch(user_id).then(user_data => {
                    var old_shards = user_data.Shards == undefined ? 0 : user_data.Shards;
                    message.channel.send(`${user_data.username}'s Redeem increased by ${amount}. Balance: ${old_shards + parseInt(amount)}`);
                });
            });
        });
    });
