...
module.exports.run = async (bot, message, args, prefix, user_available, pokemons) => {
    ...
    if (!args.length == 2) { message.channel.send(`You should specifiy slot number and pokemon ID to edit a team!`); return; }
            ...
            if (selected_team == undefined) {
                ...
                getPokemons.getallpokemon(message.author.id).then(pokemons_from_database => {
                    ...
                    if (pokemondb == undefined) {
                        return message.channel.send(`_${args[1]}_ is not a valid pokemon ID!`);
                    }
                    else {
                        if(selected_team.Pokemons.includes(pokemondb._id.toString())) return message.channel.send(`That pokemon already exists in your team!`);
